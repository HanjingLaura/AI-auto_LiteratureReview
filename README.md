 ...
 - 将 `NEXT_PUBLIC_API_BASE` 或类似环境变量用于前端 API 基地址，以便在不同部署环境下切换。
 - 若需要更实时的进度展示，可改为 SSE 或 WebSocket 推送替代轮询。

 如需我把 README 中的示例命令改为 Linux/macOS Shell 格式，或补充部署/Docker 示例，请告诉我。